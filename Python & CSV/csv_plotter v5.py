...
def xyPlot(dataList, dataLabel,col, col2=-1):
  ...
  for i in range (0,len(dataList)):
    ...
    if col2!=-1:
      trace1 = [p[1]*1000 for p in dataList[i]]
...
dataLabel = ["C = 3.938 nF","C = 3.127 nF"]
bodefiles = ["D4 bode 3.9nF.csv","D4 bode 3.1nF.csv"]
# bodeDiagram(bodefiles, dataLabel)



spektrumFiles = ["final 300k -5k.csv"]

dataLabel = ["Jord","$v_2$","$v_1$","$R_2$ = 718$\Omega$","$R_2$ = 5k$\Omega$","$R_2$ = 10,2k$\Omega$"]

spektrumDiagram(spektrumFiles,dataLabel)


faseFiles= ["D4 skop R=.7.csv","D4 skop R=304.csv","D4 skop R=718.csv","D4 skop R=5k.csv","D4 skop R=10.2k.csv"]
# dataLabel.reverse()
# faseFiles.reverse()
